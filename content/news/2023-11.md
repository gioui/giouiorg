...
For applications with simple event loops, the change may be as simple as replacing `<-w.Events()` with `w.NextEvent()`, but many Gio applications may have used event loop designs for which this simple substition isn't adequate. I'll cover how to convert each of these next.
...
If you have an event loop with a custom select with multiple cases, things get a litte more nuanced. Let's examine the simple case of having a custom select case which just triggers window invalidation:

```go
for {
    select {
    case event := range w.Events():
    	switch event := event.(type) {
    	case system.FrameEvent:
    	// Omitted
    	case system.DestroyEvent:
...
This structure can be converted pretty easily by converting the extra select case into a dedicated groutine:
